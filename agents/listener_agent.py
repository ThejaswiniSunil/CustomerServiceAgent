...
# FIX: Max complaint length to avoid sending huge strings to Gemini.
# Configurable via .env — default 2000 chars.
...
def listen(complaint: str) -> dict:
    ...
    # FIX: Input validation — three guards added:
    #   1. None input would crash .strip() with AttributeError
    #   2. Empty/whitespace-only string wastes a Gemini API call
    #   3. Very long strings (>2000 chars) are truncated before being sent
    ...
    if not complaint:
        return {
            "status": "error",
            "customer_message": (
                "We did not receive any complaint text. "
                "Please describe your issue and we will be happy to help."
            ),
            "extracted_data": {
                ...
                "complaint_summary": "Empty complaint submitted"
            }
        }
